...
export const useIndustryTypes = (page: number = 1, pageSize: number = 10) => {
  ...
  const { data: fetchedData, refetch, isLoading, isError } = useQuery({
    queryKey: ['industry_types', page, pageSize], // Thêm page và pageSize vào queryKey
    queryFn: async () => {
      if (!user?._id) {
        throw new Error('User not authenticated');
      }
      const response = await INDUSTRY_TYPE_API.getAll(
        {
          page,
          pageSize,
        },
