...
export const useOrganizationTypes = (page: number = 1, pageSize: number = 10) => {
  ...
  const { data: fetchedData, refetch, isLoading, isError } = useQuery({
    queryKey: ['organization_types', page, pageSize], // Thêm page và pageSize vào queryKey
    queryFn: async () => {
      if (!user?._id) {
        throw new Error('User not authenticated');
      }
      const response = await ORGANIZATION_TYPE_API.getAll(
        {
          page,
          pageSize,
        },
