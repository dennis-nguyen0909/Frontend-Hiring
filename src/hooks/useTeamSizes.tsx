...
import { ORGANIZATION_TYPE_API } from '../services/modules/OrganizationTypeService';
import { INDUSTRY_TYPE_API } from '../services/modules/IndustryTypeService';
import { TEAMSIZE_API } from '../services/modules/TeamSizeService';
...
export const useTeamSizes = (page: number = 1, pageSize: number = 10) => {
  const [data, setData] = useState<TeamSize[]>([]);
  ...
  const { data: fetchedData, refetch, isLoading, isError } = useQuery({
    queryKey: ['team-sizes', page, pageSize], // Thêm page và pageSize vào queryKey
    queryFn: async () => {
      if (!user?._id) {
        throw new Error('User not authenticated');
      }
      const response = await TEAMSIZE_API.getAll(
        {
          page,
          pageSize,
        },
