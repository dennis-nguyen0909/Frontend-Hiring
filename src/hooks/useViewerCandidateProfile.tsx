...
export const useViewerCandidateProfile = (page: number = 1, pageSize: number = 10) => {
  ...
  const { data: fetchedData, refetch, isLoading, isError } = useQuery({
    queryKey: ['viewer_candidate_profile_week', page, pageSize], // Thêm page và pageSize vào queryKey
    queryFn: async () => {
      if (!user?._id) {
        throw new Error('User not authenticated');
      }
      const response = await NOTIFICATION_API.getNotificationsByWeek(
        {
          page,
          pageSize,
