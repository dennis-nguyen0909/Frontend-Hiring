...
const OverviewEmployer = () => {
  ...
  const handleGetAllJobRecent =async ({current=1,pageSize=10})=>{
    try {
      const res = await JobApi.getAllJobRecent({pageSize:pageSize,current:current});
      if(res.data){
  ...
  console.log("jobRecents",jobRecents)
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-blue-50 p-6 rounded-lg">
          <div className="text-3xl font-bold">{countJobActive}</div>
          <div className="flex items-center gap-2">
            <FileTextOutlined />
            <span>Open Jobs</span>
          </div>
        ...
        <div className="bg-orange-50 p-6 rounded-lg">
          <div className="text-3xl font-bold">{metaSaveCandidate?.total && metaSaveCandidate.total || 0}</div>
          <div className="flex items-center gap-2">
            <SaveOutlined />
