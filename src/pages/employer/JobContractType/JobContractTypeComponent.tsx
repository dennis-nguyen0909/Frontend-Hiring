...
export default function JobContractTypeComponent() {
  ...
  const handleGetAllEmployerSkills = async (params?: any) => {
    try {
      ...
      const res = await JOB_CONTRACT_TYPE_API.getAll(
        newParams,
  ...
  const handleUpdate = async (values: any) => {
    const res = await JOB_CONTRACT_TYPE_API.update(
      selectedSkill?._id,
      values,
      userDetail.access_token
    );
  ...
  const handleDelete = async (record: any) => {
    const res = await JOB_CONTRACT_TYPE_API.deleteManyLevels(
      [record._id],
      userDetail.access_token
    );
    if (res.data) {
        ...
        renderBody={() => (
          ...
          >
            ...
            <Form.Item name="description" label={t("description")}>
              <Input.TextArea
                placeholder={t("enter_job_contract_type_description")}
