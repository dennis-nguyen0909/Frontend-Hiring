...
import { JOB_TYPE } from "../api/education";
import { axiosInstance } from "../config/axiosInterceptor"; 

// Định nghĩa EducationApi và hàm postEducation
export const JOB_TYPE_API = {
  create: async (data: any, accessToken: string) => {
    const res = await axiosInstance.post(`${JOB_TYPE}`, data, {
      headers: {
  ...
  getById: async (id: string, accessToken: string) => {
    const res = await axiosInstance.get(`${JOB_TYPE}/${id}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      withCredentials: true,
  ...
  update: async (id: string, data: any, accessToken: string) => {
    const res = await axiosInstance.patch(`${JOB_TYPE}/${id}`, data, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      withCredentials: true,
  ...
  getByUserId: async (accessToken: string, params: any) => {
    ...
    const res = await axiosInstance.get(`${JOB_TYPE}/user?${queryParams}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
      withCredentials: true,
  ...
  getAll:async (params:any,accessToken:string)=>{
    const res = await axiosInstance.get(`${JOB_TYPE}`,{
      params: {
        ...params
      },
      headers: {
